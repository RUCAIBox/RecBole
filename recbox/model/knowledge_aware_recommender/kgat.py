    ...
    def forward(self, norm_matrix, ego_embeddings):
        ...
        if self.aggregator_type == 'gcn':
            ego_embeddings = self.activation(
                self.W(ego_embeddings + side_embeddings))
        elif self.aggregator_type == 'graphsage':
            ego_embeddings = self.activation(
...
class KGAT(KnowledgeRecommender):
    ...
    def _get_kg_embedding(self, h, r, pos_t, neg_t):
        ...
        r_trans_w = self.trans_w(r).view(r.size(0), self.embedding_size,
                                         self.kg_embedding_size)
    ...
    def generate_transE_score(self, hs, ts, r):
        ...
        r_trans_w = self.trans_w.weight[r].view(self.embedding_size,
                                                self.kg_embedding_size)
    ...
    def update_attentive_A(self):
        ...
        for rel_idx in range(1, self.n_relations, 1):
            ...
            kg_score = self.generate_transE_score(self.all_hs[triple_index],
        ...
        A_in = torch.sparse.FloatTensor(indices, kg_score,
                                        self.matrix_size).cpu()
