...
class KTUP(KnowledgeRecommender):
    ...
    def convert_to_one_hot(self, indices, num_classes):
        ...
        one_hot = Variable(
            indices.data.new(new_shape).zero_().scatter_(
                len(old_shape), indices.data, 1))
        return one_hot
    ...
    def st_gumbel_softmax(self, logits, temperature=1.0):
        ...
        y_hard = self.convert_to_one_hot(indices=y_argmax,
                                         num_classes=y.size(len(y.shape) -
                                                            1)).float()
        y = (y_hard - y).detach() + y
        return y
    ...
    def _get_preferences(self, user_e, item_e, use_st_gumbel=False):
        pref_probs = torch.matmul(
            user_e + item_e,
            torch.t(self.pref_embedding.weight +
                    self.relation_embedding.weight)) / 2
        if use_st_gumbel:
        ...
        relation_e = torch.matmul(
            pref_probs,
            self.pref_embedding.weight + self.relation_embedding.weight) / 2
        norm_e = torch.matmul(
            pref_probs, self.pref_norm_embedding.weight +
            self.relation_norm_embedding.weight) / 2
        return pref_probs, relation_e, norm_e

    @staticmethod
    def _transH_projection(original, norm):
        return original - torch.sum(
            original * norm, dim=len(original.size()) - 1, keepdim=True) * norm
    ...
    def calculate_loss(self, interaction):
        ...
        proj_pos_user_e, pos_relation_e, proj_pos_item_e = self.forward(
            user, pos_item)
        proj_neg_user_e, neg_relation_e, proj_neg_item_e = self.forward(
            user, neg_item)

        pos_item_score = self._get_score(proj_pos_user_e, pos_relation_e,
                                         proj_pos_item_e)
        neg_item_score = self._get_score(proj_neg_user_e, neg_relation_e,
                                         proj_neg_item_e)

        rec_loss = self.rec_loss(pos_item_score, neg_item_score)
        orthogonal_loss = orthogonalLoss(self.pref_embedding.weight,
                                         self.pref_norm_embedding.weight)
        item = torch.cat([pos_item, neg_item])
        align_loss = self.align_weight * alignLoss(self.item_embedding(item),
                                                   self.entity_embedding(item),
                                                   self.L1_flag)
    ...
    def calculate_kg_loss(self, interaction):
        ...
        kg_loss = self.kg_loss(pos_tail_score, neg_tail_score,
                               torch.ones(h.size(0)).to(self.device))
        ...
        align_loss = self.align_weight * alignLoss(
            self.item_embedding(entity), self.entity_embedding(entity),
            self.L1_flag)
