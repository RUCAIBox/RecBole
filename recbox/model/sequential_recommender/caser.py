...
class Caser(SequentialRecommender):
    ...
    def forward(self, interaction):
        # Embedding Look-up
        # use unsqueeze() to get 4-D (batchsize*1*max_length*embedding_size)
        item_list_emb = self.item_list_embedding(interaction[self.ITEM_ID_LIST]).unsqueeze(1)  # use unsqueeze() to get 4-D
    ...
    def get_item_lookup_table(self):
        """Get the transpose of item_list_embedding.weight，size: (embedding_size * item_count)
