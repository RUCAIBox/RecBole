...
class SASRec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.item_list_embedding = nn.Embedding(self.item_count,
                                                self.embedding_size,
                                                padding_idx=0)
        self.position_list_embedding = nn.Embedding(max_item_list_length,
                                                    self.embedding_size)
        self.emb_dropout = nn.Dropout(self.dropout[0])
        self.multi_head_attention = MultiHeadAttention(self.n_head,
                                                       self.d_model, self.d_k,
                                                       self.d_v)
        self.layer_norm = nn.LayerNorm(self.embedding_size)
        self.conv1d_1 = nn.Conv1d(in_channels=self.d_model,
                                  out_channels=self.d_ff,
                                  kernel_size=1,
                                  bias=True)
        self.conv1d_2 = nn.Conv1d(in_channels=self.d_ff,
    ...
    def forward(self, interaction):
        ...
        position_list_emb = self.position_list_embedding(
            interaction[self.POSITION_ID])
        behavior_list_emb = item_list_emb + position_list_emb
        behavior_list_emb_drop = self.emb_dropout(behavior_list_emb)
        key_padding_mask = self.get_attn_pad_mask(
            interaction[self.ITEM_ID_LIST], interaction[self.ITEM_ID_LIST])
        look_ahead_mask = self.get_attn_subsequence_mask(
            interaction[self.ITEM_ID_LIST])
        mask = torch.gt((key_padding_mask + look_ahead_mask), 0)
        attn_weights = []
        attn_outputs = behavior_list_emb_drop
        for i in range(self.num_blocks):
            attn_outputs, attn = self.multi_head_attention(
                attn_outputs, attn_outputs, attn_outputs, mask)
            attn_weights.append(attn)
            attn_outputs = self.feedforward(attn_outputs)
        long_term_prefernce = self.gather_indexes(
            attn_outputs, interaction[self.ITEM_LIST_LEN] - 1)
