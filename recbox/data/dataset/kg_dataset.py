...
class KnowledgeBasedDataset(Dataset):
    ...
    def _get_field_from_config(self):
        ...
        self._check_field('head_entity_field', 'tail_entity_field',
                          'relation_field', 'entity_field')
    ...
    def __str__(self):
        info = [
            ...
            'The number of items that have been linked to KG: {}'.format(
                len(self.item2entity))
        ]
        return '\n'.join(info)
    ...
    def _build_feat_list(self):
        return [
            feat for feat in
            [self.inter_feat, self.user_feat, self.item_feat, self.kg_feat]
            if feat is not None
        ]
    ...
    def _load_kg(self, token, dataset_path):
        ...
        for field_type in df.columns:
            ...
            self.set_field_property(field, FeatureType.TOKEN, FeatureSource.KG,
                                    1)
    ...
    def _check_kg(self, kg):
        ...
        assert self.relation_field in kg, kg_warn_message.format(
            self.relation_field)
    ...
    def _load_link(self, token, dataset_path):
        ...
        for item_id, entity_id in zip(df[self.iid_field].values,
                                      df[self.entity_field].values):
    ...
    def _check_link(self, link):
        ...
        assert self.entity_field in link, link_warn_message.format(
            self.entity_field)
    ...
    def _get_fields_in_same_space(self):
        ...
        fields_in_same_space = [
            ...
            (self.tail_entity_field not in _) and (self.entity_field not in _)
        ]
    ...
    def _remap_ID_all(self):
        ...
        for ent_field in [self.head_entity_field, self.tail_entity_field]:
            ...
            entity_list = [
                item2id[self.entity2item[_]] if (_ in self.entity2item) and
                (self.entity2item[_] in item2id) else _ for _ in entity_list
            ]
        ...
        for field_set in fields_in_same_space:
            ...
            remap_list.append(
                (self.inter_feat, self.iid_field, FeatureType.TOKEN))
            if self.item_feat is not None:
                remap_list.append(
                    (self.item_feat, self.iid_field, FeatureType.TOKEN))
        ...
        for ent_field in [
                self.head_entity_field, self.tail_entity_field,
                self.entity_field
        ]:
            self.field2id_token[ent_field] = entity_id_token

        self.set_field_property(self.entity_field, FeatureType.TOKEN,
                                FeatureSource.KG, 1)
    ...
    def kg_graph(self, form='coo', value_field=None):
        args = [
            self.kg_feat, self.head_entity_field, self.tail_entity_field, form,
            value_field
        ]
        if form in ['coo', 'csr']:
            ...
            raise NotImplementedError(
                'kg graph format [{}] has not been implemented.')
    ...
    def _create_ckg_sparse_matrix(self, form='coo', show_relation=False):
        ...
        assert self.field2id_token[
            self.relation_field][ui_rel_id] == '[UI-Relation]'
    ...
    def _create_ckg_graph(self, form='dgl', show_relation=False):
        ...
        if show_relation:
            ...
            ui_rel = torch.full((2 * ui_rel_num, ),
                                ui_rel_id,
                                dtype=kg_rel.dtype)
    ...
    def ckg_graph(self, form='coo', value_field=None):
        if value_field is not None and value_field != self.relation_field:
            raise ValueError(
                'value_field [{}] can only be [{}] in ckg_graph.'.format(
        ...
        if form in ['coo', 'csr']:
            ...
            raise NotImplementedError(
                'ckg graph format [{}] has not been implemented.')
