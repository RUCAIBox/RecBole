...
class Dataset(object):
    def __init__(self, config, saved_dataset=None):
        ...
        self._dataloader_apis = {
            'field2type', 'field2source', 'field2id_token'
        }
    ...
    def _build_feat_list(self):
        return [
            feat for feat in [self.inter_feat, self.user_feat, self.item_feat]
            if feat is not None
        ]
    ...
    def _restore_saved_dataset(self, saved_dataset):
        ...
        if (saved_dataset is None) or (not os.path.isdir(saved_dataset)):
            raise ValueError(
                'filepath [{}] need to be a dir'.format(saved_dataset))
    ...
    def _load_data(self, token, dataset_path):
        ...
        self.user_feat = self._load_user_or_item_feat(token, dataset_path,
                                                      FeatureSource.USER,
                                                      'uid_field')
        self.item_feat = self._load_user_or_item_feat(token, dataset_path,
                                                      FeatureSource.ITEM,
                                                      'iid_field')
    ...
    def _load_inter_feat(self, token, dataset_path):
        if self.benchmark_filename_list is None:
            ...
            inter_feat = self._load_feat(inter_feat_path,
                                         FeatureSource.INTERACTION)
            self.logger.debug(
                'interaction feature loaded successfully from [{}]'.format(
                    inter_feat_path))
            ...
            for filename in self.benchmark_filename_list:
                file_path = os.path.join(
                    dataset_path, '{}.{}.{}'.format(token, filename, 'inter'))
                if os.path.isfile(file_path):
                    temp = self._load_feat(file_path,
                                           FeatureSource.INTERACTION)
    ...
    def _load_user_or_item_feat(self, token, dataset_path, source, field_name):
        feat_path = os.path.join(dataset_path,
                                 '{}.{}'.format(token, source.value))
        if os.path.isfile(feat_path):
            feat = self._load_feat(feat_path, source)
            self.logger.debug(
                'user feature loaded successfully from [{}]'.format(feat_path))
        else:
            feat = None
            self.logger.debug(
                '[{}] not found, user features are not loaded'.format(
                    feat_path))

        field = getattr(self, field_name, None)
        if feat is not None and field is None:
            raise ValueError('{} must be exist if {}_feat exist'.format(
                field_name, source.value))
        if feat is not None and field not in feat:
            raise ValueError('{} must be loaded if {}_feat is loaded'.format(
                field_name, source.value))
    ...
    def _get_load_and_unload_col(self, filepath, source):
        ...
        if self.config[
                'unload_col'] is not None and source.value in self.config[
                    'unload_col']:
        ...
        if load_col and unload_col:
            raise ValueError(
                'load_col [{}] and unload_col [{}] can not be setted the same time'
                .format(load_col, unload_col))

        self.logger.debug(
            '\n [{}]:\n\t load_col: [{}]\n\t unload_col: [{}]\n'.format(
                filepath, load_col, unload_col))
        return load_col, unload_col

    def _load_feat(self, filepath, source):
        self.logger.debug('loading feature from [{}] (source: [{}])'.format(
            filepath, source))
        ...
        for field_type in df.columns:
            ...
            try:
                ...
                raise ValueError(
                    'Type {} from field {} is not supported'.format(
                        ftype, field))
        ...
        if len(columns) == 0:
            self.logger.warning(
                'no columns has been loaded from [{}]'.format(source))
            return None
        ...
        for field in columns:
            ...
            if ftype == FeatureType.TOKEN_SEQ:
                ...
                df[field] = [
                    list(map(float, _.split(seq_separator)))
    ...
    def _user_item_feat_preparation(self):
        ...
        if self.user_feat is not None:
            new_user_df = pd.DataFrame(
                {self.uid_field: np.arange(self.user_num)})
            self.user_feat = pd.merge(new_user_df,
                                      self.user_feat,
                                      on=self.uid_field,
                                      how='left')
            flag = True
        ...
        if self.item_feat is not None:
            new_item_df = pd.DataFrame(
                {self.iid_field: np.arange(self.item_num)})
            self.item_feat = pd.merge(new_item_df,
                                      self.item_feat,
                                      on=self.iid_field,
                                      how='left')
            flag = True
    ...
    def _preload_weight_matrix(self):
        ...
        self.logger.debug('preload weight matrix for {}, drop=[{}]'.format(
            preload_fields, drop_flag))

        feats = [
            feat for feat in [self.user_feat, self.item_feat]
            if feat is not None
        ]
        for field in preload_fields:
            ...
            for feat in feats:
                if field in feat:
                    ...
                    if ftype == FeatureType.FLOAT:
                        ...
                        self.logger.warning(
                            'Field [{}] with type [{}] is not \'float\' or \'float_seq\', \
                                             which will not be handled by preload matrix.'
                            .format(field, ftype))
                        continue
            ...
            if not used_flag:
                self.logger.warning(
                    'Field [{}] doesn\'t exist, thus not been handled.'.format(
                        field))
    ...
    def _fill_nan(self):
        ...
        most_freq = SimpleImputer(missing_values=np.nan,
                                  strategy='most_frequent',
                                  copy=False)
        aveg = SimpleImputer(missing_values=np.nan,
                             strategy='mean',
                             copy=False)
        ...
        for feat in self.feat_list:
            for field in feat:
                ...
                if ftype == FeatureType.TOKEN:
                    feat[field] = most_freq.fit_transform(
                        feat[field].values.reshape(-1, 1))
                elif ftype == FeatureType.FLOAT:
                    feat[field] = aveg.fit_transform(
                        feat[field].values.reshape(-1, 1))
                elif ftype.value.endswith('seq'):
                    feat[field] = feat[field].apply(
                        lambda x: [0] if (not isinstance(x, np.ndarray) and
                                          (not isinstance(x, list))) else x)

    def _normalize(self):
        if self.config['normalize_field'] is not None and self.config[
                'normalize_all'] is not None:
            raise ValueError(
        ...
        if self.config['normalize_field']:
            ...
            for field in fields:
                ...
                if field not in self.field2type:
                    ...
                    self.logger.warning(
                        '{} is not a FLOAT/FLOAT_SEQ feat, which will not be normalized.'
                        .format(field))
    ...
    def _filter_nan_user_or_item(self):
        for field, name in zip([self.uid_field, self.iid_field],
                               ['user', 'item']):
            ...
            if feat is not None:
                ...
                if dropped_feat.any():
                    self.logger.warning(
                        'In {}_feat, line {}, {} do not exist, so they will be removed'
                        .format(name, list(dropped_feat + 2), field))
            ...
            if field is not None:
                ...
                if dropped_inter.any():
                    self.logger.warning(
                        'In inter_feat, line {}, {} do not exist, so they will be removed'
                        .format(name, list(dropped_inter + 2), field))
                    self.inter_feat.drop(self.inter_feat.index[dropped_inter],
                                         inplace=True)
    ...
    def _filter_by_inter_num(self):
        ...
        if self.user_feat is not None:
            ...
            self.user_feat.drop(self.user_feat.index[dropped_user],
                                inplace=True)

        if self.item_feat is not None:
            dropped_item = self.item_feat[self.iid_field].isin(ban_items)
            self.item_feat.drop(self.item_feat.index[dropped_item],
                                inplace=True)
        ...
        self.logger.debug('[{}] dropped interactions'.format(
            len(dropped_inter)))
        self.inter_feat.drop(self.inter_feat.index[dropped_inter],
                             inplace=True)
    ...
    def _get_illegal_ids_by_inter_num(self, field, max_num=None, min_num=None):
        ...
        ids = {
            ...
            if inter_num[id_] < min_num or inter_num[id_] > max_num
        }

        self.logger.debug('[{}] illegal_ids_by_inter_num, field=[{}]'.format(
            len(ids), field))
        return ids

    def _filter_by_field_value(self):
        filter_field = []
        filter_field += self._drop_by_value(self.config['lowest_val'],
                                            lambda x, y: x < y)
        filter_field += self._drop_by_value(self.config['highest_val'],
                                            lambda x, y: x > y)
        filter_field += self._drop_by_value(self.config['equal_val'],
                                            lambda x, y: x != y)
        filter_field += self._drop_by_value(self.config['not_equal_val'],
                                            lambda x, y: x == y)
        ...
        if self.uid_field is not None:
            remained_inter &= self.inter_feat[self.uid_field].isin(
                remained_uids)
        if self.iid_field is not None:
            remained_inter &= self.inter_feat[self.iid_field].isin(
                remained_iids)
        self.logger.debug(
            '[{}] interactions are remained after filtering'.format(
                len(remained_inter)))
        self.inter_feat.drop(self.inter_feat.index[~remained_inter],
                             inplace=True)
    ...
    def _drop_by_value(self, val, cmp):
        ...
        for field in val:
            if field not in self.field2type:
                raise ValueError(
                    'field [{}] not defined in dataset'.format(field))
            if self.field2type[field] not in {
                    FeatureType.FLOAT, FeatureType.FLOAT_SEQ
            }:
                raise ValueError(
                    'field [{}] is not float like field in dataset, which can\'t be filter'
                    .format(field))
            for feat in self.feat_list:
                if field in feat:
                    feat.drop(feat.index[cmp(feat[field].values, val[field])],
                              inplace=True)
    ...
    def _del_col(self, field):
        ...
        for dct in [
                self.field2id_token, self.field2seqlen, self.field2source,
                self.field2type
        ]:
            if field in dct:
                del dct[field]
    ...
    def _set_label_by_threshold(self):
        ...
        self.set_field_property(self.label_field, FeatureType.FLOAT,
                                FeatureSource.INTERACTION, 1)
        for field, value in threshold.items():
            if field in self.inter_feat:
                self.inter_feat[self.label_field] = (self.inter_feat[field] >=
                                                     value).astype(int)
            else:
    ...
    def _get_fields_in_same_space(self):
        ...
        for field in token_like_fields:
            ...
            if count == 0:
                ...
                raise ValueError(
                    'field [{}] occurred in `fields_in_same_space` more than one time'
                    .format(field))

        for field_set in fields_in_same_space:
            if self.uid_field in field_set and self.iid_field in field_set:
                raise ValueError(
                    'uid_field and iid_field can\'t in the same ID space')
            for field in field_set:
                if field not in token_like_fields:
                    raise ValueError(
                        'field [{}] is not a token like field'.format(field))
    ...
    def _remap_ID_all(self):
        ...
        self.logger.debug(
            'fields_in_same_space: {}'.format(fields_in_same_space))
        for field_set in fields_in_same_space:
            remap_list = []
            for field, feat in zip([self.uid_field, self.iid_field],
                                   [self.user_feat, self.item_feat]):
                if field in field_set:
                    field_set.remove(field)
                    remap_list.append(
                        (self.inter_feat, field, FeatureType.TOKEN))
                    if feat is not None:
    ...
    def set_field_property(self, field, field2type, field2source,
                           field2seqlen):
    ...
    def __str__(self):
        ...
        if self.uid_field:
            info.extend([
                'The number of users: {}'.format(self.user_num),
                'Average actions of users: {}'.format(
                    self.avg_actions_of_users)
            ])
        if self.iid_field:
            info.extend([
                'The number of items: {}'.format(self.item_num),
                'Average actions of items: {}'.format(
                    self.avg_actions_of_items)
            ])
        info.append('The number of inters: {}'.format(self.inter_num))
        if self.uid_field and self.iid_field:
            info.append('The sparsity of the dataset: {}%'.format(
                self.sparsity * 100))
    ...
    def split_by_ratio(self, ratios, group_by=None):
        self.logger.debug('split by ratios [{}], group_by=[{}]'.format(
            ratios, group_by))
        ...
        if group_by is None:
            ...
            grouped_inter_feat_index = self.inter_feat.groupby(
                by=group_by).groups.values()
            next_index = [[] for i in range(len(ratios))]
            for grouped_index in grouped_inter_feat_index:
                tot_cnt = len(grouped_index)
                split_ids = self._calcu_split_ids(tot=tot_cnt, ratios=ratios)
                for index, start, end in zip(next_index, [0] + split_ids,
                                             split_ids + [tot_cnt]):
                    index.extend(grouped_index[start:end])

        next_df = [
            self.inter_feat.loc[index].reset_index(drop=True)
    ...
    def leave_one_out(self, group_by, leave_one_num=1):
        ...
        next_index = self._split_index_by_leave_one_out(
            grouped_inter_feat_index, leave_one_num)
        next_df = [
            self.inter_feat.loc[index].reset_index(drop=True)
            for index in next_index
        ]
    ...
    def sort(self, by, ascending=True):
        self.inter_feat.sort_values(by=by,
                                    ascending=ascending,
                                    inplace=True,
                                    ignore_index=True)
    ...
    def build(self, eval_setting):
        ...
        if ordering_args['strategy'] == 'shuffle':
            ...
            self.sort(by=ordering_args['field'],
                      ascending=ordering_args['ascending'])

        group_field = eval_setting.group_field

        split_args = eval_setting.split_args
        if split_args['strategy'] == 'by_ratio':
            datasets = self.split_by_ratio(split_args['ratios'],
    ...
    def save(self, filepath):
        ...
        with open(os.path.join(filepath, 'basic-info.json'),
                  'w',
    ...
    def _create_graph(self,
        ...
        if form == 'dgl':
            ...
            graph = Data(edge_index=torch.stack([src, tgt]),
                         edge_attr=edge_attr)
            return graph
        else:
            raise NotImplementedError(
                'graph format [{}] has not been implemented.'.format(form))
    ...
    def _history_matrix(self, row, value_field=None):
        ...
        if col_num > max_col_num * 0.2:
            self.logger.warning(
                'max value of {}\'s history interaction records has reached {}% of the total'
                .format(
                    row,
                    col_num / max_col_num * 100,
                ))
        ...
        for k in data:
            ...
            if ftype == FeatureType.TOKEN:
                ...
                if isinstance(data[k], np.ndarray):
                    data[k] = torch.LongTensor(
                        data[k][:, :self.field2seqlen[k]])
                else:
                    seq_data = [
                        torch.LongTensor(d[:self.field2seqlen[k]])
                        for d in data[k]
                    ]
                    data[k] = rnn_utils.pad_sequence(seq_data,
                                                     batch_first=True)
            elif ftype == FeatureType.FLOAT_SEQ:
                if isinstance(data[k], np.ndarray):
                    data[k] = torch.FloatTensor(
                        data[k][:, :self.field2seqlen[k]])
                else:
                    seq_data = [
                        torch.FloatTensor(d[:self.field2seqlen[k]])
                        for d in data[k]
                    ]
                    data[k] = rnn_utils.pad_sequence(seq_data,
                                                     batch_first=True)
