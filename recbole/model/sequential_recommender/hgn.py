...
class HGN(SequentialRecommender):
    ...
    def reg_loss(self,user_embedding,item_embedding,seq_item_embedding):
        ...
        loss_1=reg_1 * torch.norm(self.w1.weight, p=2)+reg_1*torch.norm(self.w2.weight,p=2)+reg_1*torch.norm(self.w3.weight,p=2)+reg_1*torch.norm(self.w4.weight,p=2)
        loss_2=reg_1*torch.norm(user_embedding,p=2)+reg_1*torch.norm(item_embedding,p=2)+reg_1*torch.norm(seq_item_embedding,p=2)
    ...
    def feature_gating(self,seq_item_embedding,user_embedding):
        ...
        user_item = self.sigmoid(seq_item_embedding + user_embedding + seq_item_embedding + self.b)
        # batch_size * seq_len * embedding_size
