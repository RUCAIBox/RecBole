    ...
    def _set_default_parameters(self):
        ...
        if hasattr(self.model_class, 'input_type'):
            ...
            if self.final_config_dict['loss_type'] in ['CE']:
                if self.final_config_dict['MODEL_TYPE'] == ModelType.SEQUENTIAL and self.final_config_dict['neg_sampling'] is not None:
                    raise ValueError(f"neg_sampling [{self.final_config_dict['neg_sampling']}] should be 0 when the loss_type is CE.")
    ...
    def _set_train_neg_sample_args(self):
        ...
        if neg_sampling is None:
            ...
            if distribution not in ['uniform', 'popularity']:
                raise ValueError(f"The distribution [{distribution}] of neg_sampling should in ['uniform', 'popularity']")
