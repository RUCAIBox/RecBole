...
class Dataset(torch.utils.data.Dataset):
    ...
    def _create_sparse_matrix(
        ...
        if value_field is None:
            ...
            data = (
                df_feat[value_field][..., 0]
                if self.field2type[value_field] == FeatureType.FLOAT
                else df_feat[value_field]
            )
        ...
        if form == "dgl":
            ...
            if value_field is not None:
                ...
                for k in value_field:
                    graph.edata[k] = (
                        tensor_feat[k]
                        if self.field2type[k] != FeatureType.FLOAT
                        else tensor_feat[k][..., 0]
                    )
            return graph
        elif form == "pyg":
            ...
            edge_attr = (
                (
                    tensor_feat[value_field]
                    if self.field2type[value_field] != FeatureType.FLOAT
                    else tensor_feat[value_field][..., 0]
                )
                if value_field
                else None
            )
    ...
    def _history_matrix(self, row, value_field=None):
        ...
        if value_field is None:
            ...
            values = (
                self.inter_feat[value_field]
                if self.field2type[value_field] != FeatureType.FLOAT
                else self.inter_feat[value_field][..., 0]
            ).numpy()
