...
def get_result(name, case=0):
    ...
    return func(getattr(TestCases, f'user_len_list{case}'),
                getattr(TestCases, f'pos_len_list{case}'),
                getattr(TestCases, f'pos_rank_sum{case}'))


class TestRankMetrics(unittest.TestCase):
    def test_gauc(self):
        name = 'gauc'
        self.assertEqual(get_result(name, case=0), (1 * ((2 - (1 - 1) / 2 - 1 / 1) / (2 - 1)) +
                                                    2 * ((3 - (2 - 1) / 2 - 4 / 2) / (3 - 2)) +
                                                    3 * ((5 - (3 - 1) / 2 - 9 / 3) / (5 - 3)))
                         / (1 + 2 + 3))
        self.assertEqual(get_result(name, case=1), (3 - 0 - 3 / 1) / (3 - 1))
