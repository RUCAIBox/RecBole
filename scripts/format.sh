...
if ! [[ -e "$ROOT/.git/refs/remotes/upstream" ]]; then
    git remote add 'upstream' 'https://github.com/RUCAIBox/RecBox.git'
fi
...
tool_version_check() {
    if [[ $2 != $3 ]]; then
        echo "WARNING: RecBox uses $1 $3, You currently are using $2. This might generate different results."
    fi
}

tool_version_check "flake8" $FLAKE8_VERSION "3.7.7"
tool_version_check "yapf" $YAPF_VERSION "0.30.0"
...
format_all() {
    yapf --diff "${YAPF_FLAGS[@]}" "${YAPF_EXCLUDES[@]}" recbox
    #yapf --in-place "${YAPF_FLAGS[@]}" "${YAPF_EXCLUDES[@]}" recbox
}
