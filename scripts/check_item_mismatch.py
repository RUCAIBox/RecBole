...
# 工具函数：根据基础名匹配列（支持 typed header，如 `item_id:token`）
...
config = Config(
    ...
    config_file_list=["sasrec_baseline.yaml"],
    config_dict={"device": "cpu", "train_neg_sample_args": None, "loss_type": "CE"}
)
...
if os.path.exists(inter_path):
    ...
    print(f"\n原始交互文件: {inter_path}")
    iid_col = _find_col_by_base(inter_df, [config["ITEM_ID_FIELD"], "item_id", "item", "iid"])
    if iid_col is not None and iid_col in inter_df.columns:
        print(f"  唯一items({iid_col}): {inter_df[iid_col].nunique()}")
    else:
        print("  未找到 item_id 列（可能为typed header），可用列:")
        print(f"  {list(inter_df.columns)[:10]} ...")
    print(f"  总交互数: {len(inter_df)}")

# 5. 建议
print("\n=== 分析 ===")
if emb is not None and emb.shape[0] != dataset.num(dataset.iid_field):
